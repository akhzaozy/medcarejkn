...
async function runImport() {
  ...
  try {
    ...
    // 1. Reset Schema
    const schemaSql = fs.readFileSync(path.join(DATASET_DIR, 'schema.sql'), 'utf-8');
    await client.query(`
    ...
    // Also include sample of Ghost Enrollee from combined so it is fully represented in active cases queue
